...
from .config import DownloadConfig, DEFAULT_RESOLUTION_NUM
...
class YoutubeDownloader:
    ...
    def _extract_info(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            if "Private video" in error_msg:
                self.logger.error(f"This video is private and cannot be downloaded.")
            elif "Age restricted" in error_msg:
                self.logger.error(f"This video is age-restricted and requires login.")
            elif 'Video unavailable' in error_msg:
                self.logger.error(f"This video is unavailable.")
            else:
    ...
    def get_video_metadata(self, url: str) -> Dict:
        ...
        try:
            field_list = [
                "id", "title", "description", "channel", "upload_date", 
                "duration", "tags", "view_count", "like_count", "subtitles", "chapters"
            ]
    ...
    def _create_progress_hook(self, download_id: str):
        ...
        def progress_hook(d):
            ...
            if self.cancellation_events.get(download_id) and self.cancellation_events[download_id].is_set():
    ...
    def cleanup_progress_of_old_downloads(self, max_age_hours: int = 24):
        ...
        with self.progress_lock:
            ...
            for download_id, progress_info in self.progress_store.items():
                ...
                if status in ['pending', 'downloading', 'pausing'] or (completion_time and completion_time > cutoff_time):
        ...
        with self.progress_lock:
            self.progress_store = {
                ...
                if v.get('status') != 'completed' or v.get('completion_time', datetime.now()) > cutoff_time
            }
            to_delete = [
                k for k, v in self.progress_store.items() 
                if v.get('status') in ['completed', 'failed', 'cancelled'] and v.get('timestamp', 0) < cutoff_time]
            for k in to_delete:
    ...
    def resume_download(self, download_id: str):
        ...
        with self.progress_lock:
            ...
            if status not in ['paused', 'failed', 'cancelled']:
                self.logger.warning(f"Cannot resume '{download_id}': Download is not in a resumable state (status: {status}).")
                return
    ...
    def _build_proxy_options(self, config: Dict) -> Dict:
        ...
        """
        options = {}
        if config.get('http_proxy'): options['http_proxy'] = config['http_proxy']
        if config.get('https_proxy'): options['https_proxy'] = config['https_proxy']
        if config.get('socks5_proxy'): options['proxy'] = config['socks5_proxy']
        if config.get('no_proxy'): options['no_proxy'] = config['no_proxy']
        return options
    ...
    def start_download_task(self, download_id: str, config: DownloadConfig):
        ...
        if not utils.is_valid_url(config.url):
            ...
            with self.progress_lock:
                self.progress_store[download_id] = {'status': 'failed', 'error': 'Invalid URL format.'}
            return
        ...
        final_template = sanitized_template.replace('{current_date}', datetime.now().strftime('%Y%m%d'))
        ...
        if config.media_type == 'audio':
            ...
            try:
                ...
                self.logger.warning(f"Invalid resolution '{config.resolution}', defaulting to {DEFAULT_RESOLUTION_NUM}p.")
        ...
        if config.extract_audio:
            ytdl_opts['postprocessors'].append({'key': 'FFmpegExtractAudio', 'preferredcodec': config.audio_codec})
        if config.chapters:
            ytdl_opts['postprocessors'].append({'key': 'FFmpegSplitChapters', 'chapters': ','.join(map(str, config.chapters))})
        ...
        if config.media_type != 'audio' and config.video_format:
            ytdl_opts['postprocessors'].append({
                'key': 'FFmpegVideoConvertor', 'preferedformat': config.video_format,
            })
            ...
            try:
                ...
                while not acquired:
                    if self.cancellation_events[download_id].is_set():
                        self.logger.warning(f"Task '{download_id}' was cancelled while waiting for a download slot.")
